...
TOKEN = '6085227982:AAEd5WNNNtLzx1vmmu2N9gt7mNlWRAExLDk'
...
def format_data_analisis(data):
    ...
    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[0]} = {', '.join(str(num) for num in data[1:7])} \n0={sum(1 for num in data[1:7] if num % 2 == 0)} 1={sum(1 for num in data[1:7] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[1]} = {', '.join(str(num) for num in data[2:8])} \n0={sum(1 for num in data[2:8] if num % 2 == 0)} 1={sum(1 for num in data[2:8] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[2]} = {', '.join(str(num) for num in data[3:9])} \n0={sum(1 for num in data[3:9] if num % 2 == 0)} 1={sum(1 for num in data[3:9] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[3]} = {', '.join(str(num) for num in data[4:10])} \n0={sum(1 for num in data[4:10] if num % 2 == 0)} 1={sum(1 for num in data[4:10] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[4]} = {', '.join(str(num) for num in data[5:11])} \n0={sum(1 for num in data[5:11] if num % 2 == 0)} 1={sum(1 for num in data[5:11] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[5]} = {', '.join(str(num) for num in data[6:12])} \n0={sum(1 for num in data[6:12] if num % 2 == 0)} 1={sum(1 for num in data[6:12] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[6]} = {', '.join(str(num) for num in data[7:13])} \n0={sum(1 for num in data[7:13] if num % 2 == 0)} 1={sum(1 for num in data[7:13] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[7]} = {', '.join(str(num) for num in data[8:14])} \n0={sum(1 for num in data[8:14] if num % 2 == 0)} 1={sum(1 for num in data[8:14] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[8]} = {', '.join(str(num) for num in data[9:15])} \n0={sum(1 for num in data[9:15] if num % 2 == 0)} 1={sum(1 for num in data[9:15] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[9]} = {', '.join(str(num) for num in data[10:16])} \n0={sum(1 for num in data[10:16] if num % 2 == 0)} 1={sum(1 for num in data[10:16] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit

    pesan += f" [{my_datetime.strftime('%H:%M')}] !{data[10]} = {', '.join(str(num) for num in data[11:17])} \n0={sum(1 for num in data[11:17] if num % 2 == 0)} 1={sum(1 for num in data[11:17] if num % 2 != 0)}\n\n"
    my_datetime -= datetime.timedelta(minutes=2)  # Mengurangi 2 menit
